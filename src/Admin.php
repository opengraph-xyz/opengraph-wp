...
class Admin {
    ...
    public function save_edited_form( $post_id, $post ) {
      ...
      // Check the nonce.
      if ( empty( $_POST['opengraph-xyz-nonce'] ) || ! wp_verify_nonce( $_POST['opengraph-xyz-nonce'], 'opengraph-xyz' ) ) {
        return;
      }
      ...
      $opengraph = isset($_POST['opengraph']) ? $_POST['opengraph'] : array();
      $new      = wp_kses_post_deep( wp_unslash( $opengraph ) );
      ...
      $existing['post_types'] = isset( $new['post_types'] ) ? array_map('sanitize_text_field', $new['post_types']) : array();
